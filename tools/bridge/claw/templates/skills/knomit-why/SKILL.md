...
- `src://<source>/<path>@<commit>` — source file in source repo `<source>` at a specific commit. If `<source>` matches your session's source (named in the `knomit-guidance` skill's rendered instructions), the file may have changed since `<commit>` — use `git show <commit>:<path>` to see the version the fact was anchored to.
