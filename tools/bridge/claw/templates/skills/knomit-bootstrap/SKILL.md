...
- the `knomit-recall` skill (for this area) returned zero or near-zero facts
...
### Step 5 — Verify by running `knomit-recall` for the area again

After writing, recall and confirm the new facts appear and group sensibly. If facts didn't land where expected, the category paths likely need adjustment — use `knomit-update` to fix individual facts before they're cemented by `knomit-review`.
