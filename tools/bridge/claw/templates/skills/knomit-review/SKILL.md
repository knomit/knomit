...
## Long-running start (OpenClaw)

`knomit_review` can take 60–120s to start. Through the OpenClaw plugin it returns
immediately. If the result is `{ "status": "working", "resume": "<token>" }`, call
`knomit_review` again with `{ "resume": "<token>" }` — repeat until you receive a work
item. Do not treat a `working` response as an error or as completion.
