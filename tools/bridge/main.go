...
//
// --repo is required (unless --lens is given): knomit has no default repo.
//
...
// comparing values, so a future default could not silently trip the check.
...
func main() {
	...
	repo := flag.String("repo", "", "repository name (required unless --lens is given)")
	...
	// --lens is mutually exclusive with --repo; only an explicitly-passed --repo
	// counts, detected via flag.Visit.
	repoSet := false
	...
	// knomit has no default repo, so there is no name to fall back on. Say which
	// flag is missing rather than proxying to a repo the user never named.
	if *lens == "" && *repo == "" {
		fmt.Fprintln(os.Stderr, "knomit-bridge: --repo is required (or use --lens <name>); knomit has no default repo")
		flag.Usage()
		os.Exit(2)
	}
