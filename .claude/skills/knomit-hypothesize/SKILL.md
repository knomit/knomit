...
3. Continue with `session_id`, `item_id` and `response` (plus `binding` on the unscoped endpoint) until the queue drains. `session_id` is required on every call after the first; the result's `next` line names exactly what to pass.
