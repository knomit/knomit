...
3. Continue: `knomit_review` with `session_id`, `item_id` and `response` (plus `binding` on the unscoped endpoint). `session_id` is required on every call after the first: the binding selects the knowledge base and does not identify the session. The result's `next` line names exactly what to pass.
