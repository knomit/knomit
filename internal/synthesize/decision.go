...
func ApplyPruneDecisions(ctx context.Context,
	...
	for _, d := range decisions {
		...
		case "retract":
			...
			if _, err := gs.DeleteFile(ctx, agentBranch, d.Path, msg, "retract"); err != nil {
		...
		case "update":
			content, err := gs.ReadFile(ctx, agentBranch, d.Path)
			if err != nil {
			...
			commitHash, blobHash, err := gs.WriteFile(ctx, agentBranch, d.Path, updated, msg, "update")
			if err != nil {
				onProgress(ProgressEvent{Phase: "warn", Message: fmt.Sprintf("update write %s: %v", d.Path, err)})
				continue
			}
			if err := idx.Upsert(ctx, agentBranch, commitHash, store.NewFactRecord(f, blobHash)); err != nil {
	...
	for _, m := range merges {
		...
		commitHash, blobHash, err := gs.WriteFile(ctx, agentBranch, merged.Path(), content, msg, "subsume")
		if err != nil {
			onProgress(ProgressEvent{Phase: "warn", Message: fmt.Sprintf("merge write %s: %v", merged.Path(), err)})
			continue
		}
		_ = idx.Upsert(ctx, agentBranch, commitHash, store.NewFactRecord(merged, blobHash))
		...
		for _, src := range m.Paths {
			...
			if _, err := gs.DeleteFile(ctx, agentBranch, src, srcMsg, "retract"); err != nil {
...
func ApplyDistillDecisions(ctx context.Context,
	...
	for _, df := range synthesized {
		...
		commitHash, blobHash, err := gs.WriteFile(ctx, agentBranch, f.Path(), content, msg, "subsume")
		if err != nil {
			onProgress(ProgressEvent{Phase: "warn", Message: fmt.Sprintf("distill write %s: %v", f.Path(), err)})
			continue
		}
		_ = idx.Upsert(ctx, agentBranch, commitHash, store.NewFactRecord(f, blobHash))
	...
	for _, path := range retract {
		...
		if _, err := gs.DeleteFile(ctx, agentBranch, path, msg, "retract"); err != nil {
