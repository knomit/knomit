...
func bootKnomitWithLegacyOnlyOntology(t *testing.T, staleYAML string) (dir, agentBranch string) {
	...
	require.NoError(t, m.Start())
	// Start creates nothing: knomit has no default repo, so the fixture makes
	// its own. Create seeds the ontology at the canonical path, which is what
	// the DeleteFact below then removes.
	ri := mustCreateRepo(t, m, testRepoName)
...
func bootKnomitWithBothOntologies(t *testing.T, canonicalYAML, legacyYAML string) (dir, agentBranch string) {
	...
	require.NoError(t, m.Start())
	// Start creates nothing: knomit has no default repo, so the fixture makes
	// its own before seeding both ontology locations into it.
	ri := mustCreateRepo(t, m, testRepoName)
