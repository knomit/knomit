...
// no row at all: it only moved the database back and re-opened the repo,
// neither of which writes to the registry. The repo came back live and
// UNREGISTERED, which the next Start cannot see (the registry is authoritative
// and the disk is no longer globbed), so it would simply not come back after a
// restart, with nothing in the log saying why. The origin write-through cannot
// repair it either, since RecordOrigin declines to write when the row is absent.
//
