			...
			mcpgo.Items(map[string]any{
				...
				"properties": map[string]any{
					...
					"refs": map[string]any{"type": "array", "items": map[string]any{"type": "string"}, "description": "References, in four forms. " +
						"(1) A fact in THIS repo: bare `kb/<topic>/…/<id>.md`, or canonical `kb://<repo-id>/<path>`. It MUST already exist, or be written in this same call — all facts in one call are committed together, so they may cite each other in any order, including circularly. Citing a fact that will not exist REJECTS the whole call. " +
						"(2) A fact in ANOTHER repo: `kb://<repo-id>/<path>`. Never checked. " +
						"(3) Source code: `src://<source-repo-id>/<path>@<commit>:<blob>`, with FULL 40-hex commit and blob, optionally `#L<start>-L<end>`. Produce the three components with `git rev-list --max-parents=0 HEAD | cut -c1-12` (repo id), `git rev-parse HEAD` (commit), `git rev-parse <commit>:<path>` (blob). That last command failing IS the check — the server holds no source objects and cannot verify src refs for you, so never cite source that does not exist in the repo's history. The older `src://<name>/<path>@<commit>` form is still accepted. " +
