...
- **knomit_changes**: what is different under a folder (prefix) since a commit you saw (since). Returns each fact path as added/modified/deleted plus head; keep head yourself and pass it as since next time — knomit stores nothing for you. Omit since for the folder as it is now. Net difference only (an add-then-remove between two reads is not reported); a rename is deleted + added; reads main, so your own writes appear once they reach main. A since that is not behind head is refused rather than reported as deletions.
