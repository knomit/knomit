...
//
// The second assertion is not cosmetic: the Manage Logs tab reads a console
// line as `<stamp> <LVL> <message>` (web/src/LogView.tsx parseLine), so a
// dated-but-spaced format like "Jan 2 15:04:05" would fix the date and silently
// break the tab's level filter — an unparseable line is treated as unrankable
// and shown at every threshold, so the filter stops filtering rather than
// emptying.
func TestFileSinkTimestampIsDatedAndSpaceFree(t *testing.T) {
	...
	if fields[1] != "INF" {
		t.Errorf("level token = %q, want INF as the SECOND token: the Manage Logs tab's parseLine reads fields[1], "+
