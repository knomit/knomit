...
func TestStore_UnknownAndWrongKind(t *testing.T) {
	...
	if _, err := s.Refresh(ctx, out.Access); !errors.Is(err, ErrUnknownToken) {
...
func TestStore_RevokeKillsTheFamilyAndIsIdempotent(t *testing.T) {
	for _, which := range []string{"access", "refresh"} {
		t.Run(which, func(t *testing.T) {
			...
			if _, err := s.Refresh(ctx, out.Refresh); !errors.Is(err, ErrRevoked) {
...
func TestStore_RefreshRotatesAndReuseRevokesFamily(t *testing.T) {
	...
	if _, err := s.Refresh(ctx, first.Refresh); !errors.Is(err, ErrReused) {
	...
	if _, err := s.Refresh(ctx, second.Refresh); !errors.Is(err, ErrRevoked) {
...
func TestStore_RefreshLifetimeIsAbsolute(t *testing.T) {
	...
	next, err := s.Refresh(ctx, out.Refresh)
	if err != nil {
	...
	c.add(time.Hour)
	if _, err := s.Refresh(ctx, next.Refresh); !errors.Is(err, ErrExpired) {
...
func TestStore_ConcurrentRefreshOneWinner(t *testing.T) {
	...
	for i := range errs {
		...
		go func(i int) {
			defer wg.Done()
			_, errs[i] = s.Refresh(context.Background(), out.Refresh)
		}(i)
	}
	wg.Wait()
	wins := 0
