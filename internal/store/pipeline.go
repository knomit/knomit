...
func (idx *Index) GetPipelineWatermark(tool, branch string) (string, error) {
	var hash string
	err := idx.db.QueryRow(
...
func (idx *Index) SetPipelineWatermark(tool, branch, hash string) error {
	_, err := idx.db.Exec(
...
func (idx *Index) CreatePipelineSession(tool, branch string) (*PipelineSession, error) {
...
func (idx *Index) GetPipelineSession(id string) (*PipelineSession, error) {
...
func (idx *Index) CompletePipelineSession(id string) error {
	now := time.Now().UTC().Format(time.RFC3339)
	_, err := idx.db.Exec(
...
func (idx *Index) InsertPipelineWorkItem(item PipelineWorkItem) error {
	now := time.Now().UTC().Format(time.RFC3339)
	_, err := idx.db.Exec(
...
func (idx *Index) NextPipelineWorkItem(sessionID string) (*PipelineWorkItem, error) {
...
func (idx *Index) SetPipelineWorkItemResponse(id int64, response string) error {
	_, err := idx.db.Exec(
...
func (idx *Index) GCPipelineSessions(tool, branch string, keep int) error {
	_, err := idx.db.Exec(
...
func (idx *Index) PipelineWorkItemStats(sessionID string) (completed, remaining int, err error) {
	err = idx.db.QueryRow(
