...
func setupBranch(t *testing.T, idx *Index, name string) int64 {
	t.Helper()
	id, err := idx.EnsureBranch(name, "refs/heads/"+name)
	if err != nil {
...
func TestLastCommitForPath_ReturnsLatestNonDeleted(t *testing.T) {
	...
	hash, ok := idx.LastCommitForPath("test-branch", "kb/test.md")
	if !ok {
...
func TestLastCommitForPath_ExcludesDeleted(t *testing.T) {
	...
	_, ok := idx.LastCommitForPath("test-branch", "kb/gone.md")
	if ok {
...
func TestLastCommitForPath_NotFound(t *testing.T) {
	...
	_, ok := idx.LastCommitForPath("test-branch", "kb/nope.md")
	if ok {
...
func TestLastCommitForPath_BranchScoped(t *testing.T) {
	...
	hash, ok := idx.LastCommitForPath("branch-a", "kb/shared.md")
	if !ok {
	...
	hash, ok = idx.LastCommitForPath("branch-b", "kb/shared.md")
	if !ok {
...
func TestLastCommitForPath_FallbackToNullBranchID(t *testing.T) {
	...
	hash, ok := idx.LastCommitForPath("my-branch", "kb/old.md")
	if !ok {
