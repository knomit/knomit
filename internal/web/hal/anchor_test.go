...
func TestURLBuilder_APIRoot(t *testing.T) {
	...
	if got := b.APIRoot(); got != "/api/v1" {
		t.Errorf("got %q", got)
	}
	if got := b.Repos(); got != "/api/v1/repos" {
		t.Errorf("got %q", got)
	}
	if got := b.Repo("alpha"); got != "/api/v1/repos/alpha" {
		t.Errorf("got %q", got)
	}
	if got := b.Branches("alpha"); got != "/api/v1/repos/alpha/branches" {
		t.Errorf("got %q", got)
	}
}
...
func TestURLBuilder_BranchEncodesSlash(t *testing.T) {
	...
	if got := b.Branch("alpha", a); got != "/api/v1/repos/alpha/branches/agent:test" {
		t.Errorf("got %q", got)
	}
}
...
func TestURLBuilder_FactAtHEAD(t *testing.T) {
	...
	want := "/api/v1/repos/alpha/branches/agent:test/facts/know/ai/ml/abc12345.md"
	if got != want {
...
func TestURLBuilder_FactAtCommit_CarriesShaSegment(t *testing.T) {
	...
	got := b.Fact("alpha", a, "know/ai/ml/abc12345.md")
	want := "/api/v1/repos/alpha/branches/agent:test/commits/abc123/facts/know/ai/ml/abc12345.md"
	if got != want {
		t.Errorf("got %q, want %q", got, want)
	}
}
...
func TestURLBuilder_FactSubResources(t *testing.T) {
	...
	a := Anchor{Branch: "agent/test"}
	path := "know/a.md"
	if got := b.FactIncoming("alpha", a, path); got != "/api/v1/repos/alpha/branches/agent:test/facts/know/a.md/incoming" {
		t.Errorf("incoming: %q", got)
	}
	if got := b.FactOutgoing("alpha", a, path); got != "/api/v1/repos/alpha/branches/agent:test/facts/know/a.md/outgoing" {
		t.Errorf("outgoing: %q", got)
	}
	if got := b.FactCommits("alpha", a, path); got != "/api/v1/repos/alpha/branches/agent:test/facts/know/a.md/commits" {
