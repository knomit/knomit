...
it('shows Library when live, TimelineNav when history', () => {
  // Built from `init` rather than a hand-picked subset: LeftPanel reads more of
  // the state than it used to (canGoBack for the timeline's back button), and a
  // partial literal makes this test fail for reasons that have nothing to do
  // with which layer is showing.
  const base = { ...init, context: { kind: 'repo', repo: 'r' } } as any;
