...
export const api = {
  ...
  search: (q: string, minConfidence = 0): Promise<{ results: SearchResult[] }> => fetch(`${BASE}/search?q=${encodeURIComponent(q)}&min_confidence=${minConfidence}&limit=50`).then(r => r.json()),
