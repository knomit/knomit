...
export function TopBar({ state, repos, dispatch, onSettingsClick }: Props) {
  return (
    <div style={{ height: 40, background: '#111', borderBottom: '1px solid #333', display: 'flex', alignItems: 'center', padding: '0 16px', gap: 12, flexShrink: 0 }}>
      ...
      <button
        onClick={() => api.rebuild(state.repo)}
        disabled={state.tasks.rebuild?.status === 'running'}
        title="Rebuild index"
        style={{ background: 'none', border: 'none', color: state.tasks.rebuild?.status === 'running' ? '#555' : '#666', cursor: state.tasks.rebuild?.status === 'running' ? 'default' : 'pointer', padding: 4, display: 'flex', alignItems: 'center', fontSize: 11 }}
        onMouseEnter={e => { if (state.tasks.rebuild?.status !== 'running') e.currentTarget.style.color = '#aaa'; }}
        onMouseLeave={e => { if (state.tasks.rebuild?.status !== 'running') e.currentTarget.style.color = '#666'; }}
      >
        <svg width="13" height="13" viewBox="0 0 16 16" fill="currentColor">
