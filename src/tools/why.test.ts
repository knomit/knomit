...
beforeEach(async () => {
  ...
  await learnHandler(repo, {
    ...
    facts: [
      ...
      {
        path: "know/people/alice/bought-album.md",
        domain: ["personal", "music"],
        confidence: 0.9,
        sources: 1,
...
describe("knomit_why", () => {
  it("returns fact details, learning moment, and history", async () => {
    ...
    expect(result.fact.file).toBe("know/people/alice/likes-rock.md");
  ...
  it("finds sibling facts from the same learning moment", async () => {
    ...
    expect(siblingFiles).toContain("know/people/alice/bought-album.md");

    const sibling = result.learning_moment.siblings.find(s => s.file === "know/people/alice/bought-album.md");
  ...
  it("returns empty result for nonexistent file", async () => {
    ...
    expect(result.fact.file).toBe("know/nonexistent.md");
